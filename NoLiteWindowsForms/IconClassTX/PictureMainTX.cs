...
{
    public class PictureMainTX:DeviceTX
    {
        ...
        public PictureBox CreatePictureMain(int i, SerialPort port, string devicesChannel,string devicesName, TabPage tabPage, int positionPictureTop, int positionPictureLeft,string deviceTypeTx)
        {
            ...
            {
                pct.MouseClick += delegate (object sender, MouseEventArgs e) { MenuItemRGB_Setting(sender, e, port,devicesChannel,devicesName); };
                pct.MouseUp += delegate (object sender, MouseEventArgs e) { Btn_MouseUp(sender, e, port, devicesChannel, pct, devicesName, tabPage); };
            }
            else
            {
                pct.MouseClick += delegate (object sender, MouseEventArgs e) { DbClick_Connection(sender, e, port, devicesChannel); };
                pct.MouseUp += delegate (object sender, MouseEventArgs e) { Btn_MouseUp(sender, e, port, devicesChannel, pct, devicesName, tabPage); };
            }
